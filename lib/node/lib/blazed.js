  ...
  return new Promise((resolve, reject) => {
    ...
    const send = urlParser.parseThisURL(url).then(parsedURL => {
      ...
      if (!requestOptions.headers['User-Agent']) {
        ...
        requestOptions.headers['User-Agent'] = `${packageJson.name}/${packageJsonFile.version}`;
      }
      if (data) {
      ...
      request.on('error', (error) => {
        if (error.code === 'ENOTFOUND') {
          ...
          err.message = `Failed to resolve the hostname of '${url}'. Please check the URL for typos or ensure that the hostname is correct.`;
          return reject(err);
        } else if (error.code === 'ETIMEDOUT') {
          const err = new Error(`Request Timeout`);
          err.code = error.code;
          err.name = "RequestTimeoutError";
          err.message = `The HTTP ${method} request to ${url} timed out. Please check the server's response time or consider increasing the timeout value.`;
          return reject(err);
...
*/
   ...
   * @memberof Blazed
   * @param {"beforeRequest" | "redirect" | "afterRequest"} event - The event to listen to.
