...
const _makeRequest = (method, url, data, headers = {}, redirectCount = 5) => {
    ...
    (async () => {
      try {
        ...
        request.on('error', async (error) => {
          ...
          return await utilErrors.processError(error, url, null, null, null, method);
        });
        if (data) {
  ...
  response.on('end', async () => {
    ...
    if (contentType?.includes('application/json')) {
      try {
        ...
        responseObject.responseSize = response.headers['content-length'] ? formatBytes(response.headers['content-length']) : formatBytes(Buffer.byteLength(parsedData));
        for (const key in response.headers) {
          responseObject.responseHeaders[key] = response.headers[key];
        }
        // Emitter for the 'afterRequest' event
      ...
      responseObject.responseSize = response.headers['content-length'] ? formatBytes(response.headers['content-length']) : formatBytes(Buffer.byteLength(responseData));
      responseObject.status = response.statusCode;
      // Emitter for the 'afterRequest' event
      emitter.emit("afterRequest", originalUrl, responseObject);
      return resolve(responseObject);
...
const about = () => {
  if (!packageJson) throw new Error(`package.json files seems to be missing please try again by downloading blazed.js again by doing ''npm i blazed.js'' in your terminal!`);
  const aboutObject = {
...
*/
  get: (url, headers, redirectCount) => _makeRequest(HTTP_METHODS.GET, url, null, headers, redirectCount),
 ...
 * @returns {Promise<{ data: *, status: number, responseHeaders: { [key: string]: string }, requestHeaders: { [key: string]: string }, responseSize: string }>} A promise that resolves with the response data.
*/
  head: (url, headers, redirectCount) => _makeRequest(HTTP_METHODS.HEAD, url, null, headers, redirectCount),
 ...
 * @returns {Promise<{ data: *, status: number, responseHeaders: { [key: string]: string }, requestHeaders: { [key: string]: string }, responseSize: string }>} A promise that resolves with the response data.
*/
  post: (url, data, headers, redirectCount) => _makeRequest(HTTP_METHODS.POST, url, data, headers, redirectCount),
 ...
 * @returns {Promise<{ data: *, status: number, responseHeaders: { [key: string]: string }, requestHeaders: { [key: string]: string }, responseSize: string }>} A promise that resolves with the response data.
*/
  put: (url, data, headers, redirectCount) => _makeRequest(HTTP_METHODS.PUT, url, data, headers, redirectCount),
 ...
 * @returns {Promise<{ data: *, status: number, responseHeaders: { [key: string]: string }, requestHeaders: { [key: string]: string }, responseSize: string }>} A promise that resolves with the response data.
*/
  delete: (url, headers, redirectCount) => _makeRequest(HTTP_METHODS.DELETE, url, null, headers, redirectCount),
 ...
 * @returns {Promise<{ data: *, status: number, responseHeaders: { [key: string]: string }, requestHeaders: { [key: string]: string }, responseSize: string }>} A promise that resolves with the response data.
*/
  connect: (url, headers, redirectCount) => _makeRequest(HTTP_METHODS.CONNECT, url, null, headers, redirectCount),
 ...
 * @returns {Promise<{ data: *, status: number, responseHeaders: { [key: string]: string }, requestHeaders: { [key: string]: string }, responseSize: string }>} A promise that resolves with the response data.
*/
  options: (url, headers, redirectCount) => _makeRequest(HTTP_METHODS.OPTIONS, url, null, headers, redirectCount),
 ...
 * @returns {Promise<{ data: *, status: number, responseHeaders: { [key: string]: string }, requestHeaders: { [key: string]: string }, responseSize: string }>} A promise that resolves with the response data.
*/
  trace: (url, headers, redirectCount) => _makeRequest(HTTP_METHODS.TRACE, url, null, headers, redirectCount),
...
*/
  patch: (url, data, headers) => _makeRequest(HTTP_METHODS.PATCH, url, data, headers),
...
*/
  request: (object) => {
    ...
    return _makeRequest(method, url, data, headers, redirectCount);
  },
  parse,
  status_codes,
  methods,
